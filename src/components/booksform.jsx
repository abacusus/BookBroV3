...
const BookSaleForm = ({user}) => {
  ...
  const uploadImageToCloudinary = async (file) => {
    ...
    const res = await fetch("https://api.cloudinary.com/v1_1/dcajb02df/image/upload", {
      method: "POST",
      body: formData,
    });
  ...
  const handleNext = async (e) => {
    ...
    if (step === questions.length - 1) {
      try {
        ...
        Listed_On: new Date().toLocaleDateString("en-GB", {
    day: "numeric",
    month: "long",
    year: "numeric",
  }), // e.g., "24 June 2025"
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div>
            <h2 className="text-5xl md:text-4xl font-bold text-indigo-600 mb-6">
              {current.label}
            </h2>
            {current.helpText && (
          ...
          <div className="w-full max-w-xl space-y-6">
            {(current.type === "text" || current.type === "number") && (
              <input
                ...
                name={current.id}
                required
                className="w-full p-4 border rounded-lg focus:ring-2 focus:ring-indigo-400 text-3xl"
            ...
            {current.type === "textarea" && (
              <textarea
                name={current.id}
                required
                className="w-full p-4 border rounded-lg focus:ring-2 focus:ring-indigo-400 text-2xl"
                rows="3"
